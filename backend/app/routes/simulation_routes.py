...
def calculate(company_id):
    ...
    answer = process_simulation(user_id, company_id, data)
...
def create(company_id):
    ...
    answer, status_code = save_simulation(current_user_id, company_id, data)
...
def get_all(company_id):
    current_user_id = int(get_jwt_identity())
    answer, status_code = get_simulation(current_user_id,company_id)
    return jsonify(answer), status_code
...
def delete(company_id, simulation_id):
    current_user_id = int(get_jwt_identity())
    answer, status_code = delete_simulation(current_user_id, company_id, simulation_id)
    return jsonify(answer), status_code