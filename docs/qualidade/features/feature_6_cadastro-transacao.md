...
| :--: | --- | --- | --- | --- | :--: |
| TS-01 | `POST /api/transactions/` com dados válidos | Integração | `201` + `transaction_id` | Conforme | ✅ |
| TS-02 | `POST` sem token | Integração | `401` | Conforme | ✅ |
| TS-03 | `POST` com valor negativo | Integração | `400` (validação) | Conforme | ✅ |
| TS-04 | `POST` com valor zero | Integração | `400` (valor estritamente positivo) | Conforme | ✅ |
| TS-05 | `POST` com data futura | Integração | `400` (data não futura) | Conforme | ✅ |
| TS-06 | `POST` sem categoria | Integração | `400` (categoria obrigatória) | Conforme | ✅ |
| TS-07 | `POST` com categoria de outra empresa | Integração | `400` (não pertence à empresa) | Conforme | ✅ |
| TS-08 | Transação recém-criada aparece no histórico | Integração | `200` + transação listada | `500` `Decimal - float` (DEF-01) | ⏭️ xfail |
| TS-09 | `PUT` edita transação existente | Integração | `200` + persistência confirmada | Conforme (relido do banco) | ✅ |
| TS-10 | `PUT` em transação inexistente | Integração | `404` | Conforme | ✅ |
| TS-11 | `PUT` sem `company_id` no corpo | Integração | `400` | Conforme | ✅ |
| TS-12 | `DELETE` exclui transação | Integração | `200` + sai do histórico | Conforme | ✅ |
| TS-13 | `DELETE` em transação inexistente | Integração | `404` | Conforme | ✅ |
| TS-14 | `DELETE` sem `company_id` | Integração | `400` | Conforme | ✅ |
| TS-15 | Usuário sem vínculo cria transação em empresa alheia | Integração | `403` | `201` — `user_id` ignorado (DEF-02) | ⏭️ xfail |
| TS-16 | `create_transaction` com categoria inexistente | Unitário | `400` | Conforme | ✅ |
| TS-17 | `create_transaction` com dados válidos | Unitário | `201` + `add`/`commit` | Conforme | ✅ |
| TS-18 | `update_transaction` em transação inexistente | Unitário | `404` | Conforme | ✅ |
| TS-19 | `delete_transaction` em transação inexistente | Unitário | `404` | Conforme | ✅ |
| TS-20 | Modal exibe título "Nova Transação" | Unitário (front) | título de criação | Conforme | ✅ |
| TS-21 | Modal exibe título "Editar Transação" ao editar | Unitário (front) | título de edição | Conforme | ✅ |
| TS-22 | Submeter formulário válido chama `onSalvar` | Unitário (front) | `valor`/`categoriaId` numéricos | Conforme | ✅ |
| TS-23 | Valor negativo bloqueia envio | Unitário (front) | erro + `onSalvar` não chamado | Conforme | ✅ |
| TS-24 | Data futura bloqueia envio | Unitário (front) | erro + `onSalvar` não chamado | Conforme | ✅ |
| TS-25 | Categoria obrigatória bloqueia envio | Unitário (front) | erro + `onSalvar` não chamado | Conforme | ✅ |
| TS-26 | Select de categoria filtra pelo tipo selecionado | Unitário (front) | só categorias do tipo | Conforme | ✅ |
| TS-27 | Botão Cancelar chama `onFechar` | Unitário (front) | `onFechar` chamado | Conforme | ✅ |
