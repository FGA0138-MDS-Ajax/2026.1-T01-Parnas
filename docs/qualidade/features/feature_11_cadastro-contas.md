...
| :--: | --- | --- | --- | --- | :--: |
| TS-01 | `_get_company_id` retorna a empresa do usuário | Unitário | `company_id` do usuário | Conforme | OK |
| TS-02 | `create_bill` persiste e retorna id | Unitário | `201` + `id`; `add`/`commit` chamados | Conforme | OK |
| TS-03 | `update_bill` em conta inexistente | Unitário | `404` | Conforme | OK |
| TS-04 | `update_bill` em conta quitada | Unitário | `400`, sem `commit` | Conforme | OK |
| TS-05 | `delete_bill` em conta inexistente | Unitário | `404` | Conforme | OK |
| TS-06 | `delete_bill` em conta quitada | Unitário | `400`, sem `delete` | Conforme | OK |
| TS-07 | `pay_bill` em conta inexistente | Unitário | `404` | Conforme | OK |
| TS-08 | `pay_bill` em conta já quitada | Unitário | `400` | Conforme | OK |
| TS-09 | `POST /api/contas` com dados válidos | Integração | `201` + `id` | Conforme | OK |
| TS-10 | `POST /api/contas` com campo obrigatório faltando | Integração | `400` | Conforme | OK |
| TS-11 | `POST /api/contas` sem token | Integração | `401` | Conforme | OK |
| TS-12 | `GET /api/contas` lista as contas da empresa | Integração | `200` + 2 contas | Conforme | OK |
| TS-13 | `GET /api/contas?status=` filtra pendentes/quitadas | Integração | só as do status pedido | Conforme | OK |
| TS-14 | `PUT /api/contas/<id>` edita conta pendente | Integração | `200` | Conforme | OK |
| TS-15 | `PUT /api/contas/<id>` em conta inexistente | Integração | `404` | Conforme | OK |
| TS-16 | `DELETE /api/contas/<id>` exclui conta pendente | Integração | `200` + some da lista | Conforme | OK |
| TS-17 | `DELETE /api/contas/<id>` em conta inexistente | Integração | `404` | Conforme | OK |
| TS-18 | `PUT /api/contas/<id>` em conta quitada | Integração | `400` (bloqueado) | Conforme | OK |
| TS-19 | `DELETE /api/contas/<id>` em conta quitada | Integração | `400` (bloqueado) | Conforme | OK |
| TS-20 | `PATCH /api/contas/<id>/quitar` em conta já quitada | Integração | `400` | Conforme | OK |
| TS-21 | `PATCH /api/contas/<id>/quitar` em conta pendente gera transação | Integração | `200` + transação criada | `IntegrityError` (`transaction.type` NOT NULL) | xfail (DEF-01) |
| TS-22 | Lista contas separadas em Pendentes e Quitadas | Unitário (front) | duas seções com as contas | Conforme | OK |
| TS-23 | Conta quitada não exibe ações (quitar/editar/excluir) | Unitário (front) | só selo "Quitada" | Conforme | OK |
| TS-24 | Quitar pendente pede confirmação e chama o serviço | Unitário (front) | modal + `liquidarConta(id)` | Conforme | OK |
| TS-25 | Excluir pendente: "Cancelar" não chama o serviço | Unitário (front) | `removerConta` não chamado | Conforme | OK |
