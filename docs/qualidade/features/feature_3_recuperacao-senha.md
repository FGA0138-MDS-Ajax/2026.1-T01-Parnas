...
|-------|--------------------------------------------------------------------------|------------|------------------------------------------------|-----------|--------|
| TS-13 | Recuperação de senha via token temporário (gera token → redefine)        | Integração | `200` + "Senha redefinida com sucesso"         | Conforme  | ✅      |
| CT-01 | `POST /auth/forgot-password` com e-mail cadastrado                        | Integração | `200` + `reset_link` e `email` no corpo        | Conforme  | ✅      |
| CT-02 | `POST /auth/forgot-password` com e-mail não cadastrado                    | Integração | `404` + "Usuário não encontrado"               | Conforme  | ✅      |
| CT-03 | `POST /auth/forgot-password` sem o campo e-mail                           | Integração | `400` + "Email obrigatório"                     | Conforme  | ✅      |
| CT-04 | `POST /auth/reset-password` com token válido                             | Integração | `200` + "Senha redefinida com sucesso"         | Conforme  | ✅      |
| CT-05 | Login passa a funcionar com a senha nova (e a antiga deixa de valer)     | Integração | `401` na senha antiga, `200` + JWT na nova      | Conforme  | ✅      |
| CT-06 | `POST /auth/reset-password` com token inválido                           | Integração | `400` + "Token inválido ou expirado"            | Conforme  | ✅      |
| CT-07 | `POST /auth/reset-password` sem campos obrigatórios                      | Integração | `400` + "Token e nova senha obrigatórios"       | Conforme  | ✅      |
| CT-08 | `generate_reset_token` + `verify_reset_token` (round-trip do e-mail)     | Unitário   | token gera e o e-mail é recuperado idêntico     | Conforme  | ✅      |
| CT-09 | `verify_reset_token` com token expirado (`expiration=-1`)               | Unitário   | retorna `None`                                  | Conforme  | ✅      |
| CT-10 | `verify_reset_token` com token forjado/inválido                         | Unitário   | retorna `None`                                  | Conforme  | ✅      |
