...
| :--: | --- | --- | --- | --- | :--: |
| TS-01 | `get_period_dates` período mensal | Unitário | 1º ao último dia do mês | Conforme | ✅ |
| TS-02 | `get_period_dates` período anual | Unitário | 01/01 a 31/12 | Conforme | ✅ |
| TS-03 | `get_period_dates` período personalizado | Unitário | usa `start`/`end` informados | Conforme | ✅ |
| TS-04 | `get_period_dates` mensal sem mês/ano | Unitário | `ValueError` | Conforme | ✅ |
| TS-05 | `get_period_dates` `start` > `end` | Unitário | `ValueError` | Conforme | ✅ |
| TS-06 | `get_period_dates` sem parâmetros | Unitário | `ValueError` | Conforme | ✅ |
| TS-07 | `GET /api/reports` sem token | Integração | `401` | Conforme | ✅ |
| TS-08 | `GET /api/reports` sem `cnpj` | Integração | `400` | Conforme | ✅ |
| TS-09 | `GET /api/reports` com `period` inválido | Integração | `400` | Conforme | ✅ |
| TS-10 | `GET /api/reports` empresa inexistente | Integração | `404` | Conforme | ✅ |
| TS-11 | `GET /api/reports` usuário sem acesso à empresa | Integração | `403` | Conforme | ✅ |
| TS-12 | `GET /api/reports` mensal sem mês/ano | Integração | `400` | Conforme | ✅ |
| TS-13 | `GET /api/reports` período invertido (`start`>`end`) | Integração | `400` | Conforme | ✅ |
| TS-14 | `GET /api/reports` mensal retorna estrutura e período | Integração | `200` + `periodo/totais/distribuicao/evolucao` | Conforme | ✅ |
| TS-15 | `GET /api/reports` período personalizado | Integração | `200` + datas corretas | Conforme | ✅ |
| TS-16 | Totais refletem as transações do período | Integração | receitas/despesas/saldo corretos | Totais zerados (filtro `ENTRADA/SAIDA`) | ⏭️ xfail (DEF-02) |
| TS-17 | Front exibe cards de Receitas, Despesas e Saldo | Unitário (front) | três cards na tela | Conforme | ✅ |
| TS-18 | Front (mensal) exibe campo de mês | Unitário (front) | `input[type=month]` | Conforme | ✅ |
| TS-19 | Front (personalizado) exibe dois campos de data | Unitário (front) | dois `input[type=date]` | Conforme | ✅ |
| TS-20 | Front (anual) exibe campo de ano | Unitário (front) | campo "Ano" | Conforme | ✅ |
