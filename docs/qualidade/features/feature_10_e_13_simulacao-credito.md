...
|-------|-------------------------------------------------------------------|------------------|---------------------------------------|-----------|--------|
| TS-01 | `calculate_table_price` com taxa 0                                | Unitário         | parcela = valor/prazo, juros 0        | Conforme  | ✅      |
| TS-02 | `calculate_table_price` parcelas iguais, saldo final zera         | Unitário         | parcela constante, saldo ≈ 0          | Conforme  | ✅      |
| TS-03 | `calculate_table_sac` amortização constante, saldo zera           | Unitário         | amortização igual, saldo ≈ 0          | Conforme  | ✅      |
| TS-04 | `calculate_table_sac` parcelas decrescentes                       | Unitário         | parcelas em ordem decrescente         | Conforme  | ✅      |
| TS-05 | SAC paga menos juros que o Price (mesmos parâmetros)              | Unitário         | juros SAC < juros Price               | Conforme  | ✅      |
| TS-06 | `process_simulation` Price — resumo coerente                      | Unitário         | total_juros = total − valor, 12 parc. | Conforme  | ✅      |
| TS-07 | `process_simulation` sem `company_id` não projeta fluxo           | Unitário         | sem `projecao_fluxo_caixa`            | Conforme  | ✅      |
| TS-08 | `process_simulation` com `company_id` anexa projeção              | Unitário         | inclui projeção (status "Indisponível")| Conforme  | ✅      |
| TS-09 | `project_impact_cash_flow` sem transações                         | Unitário         | status "Indisponível", `perc` None    | Conforme  | ✅      |
| TS-10 | `project_impact_cash_flow` empresa lucrativa                      | Unitário         | status "Saudável", perc definido      | Conforme  | ✅      |
| TS-11 | `project_impact_cash_flow` empresa no prejuízo                    | Unitário         | status "Alerta vermelho", perc 100    | Conforme  | ✅      |
| TS-12 | `POST /simulations/calculate` Price não persiste                  | Integração       | `200` + detalhamento, lista vazia     | Conforme  | ✅      |
| TS-13 | `POST /simulations/calculate` modalidade inválida                 | Integração       | `400`                                 | Conforme  | ✅      |
| TS-14 | `POST /simulations/calculate` valor negativo                      | Integração       | `400`                                 | Conforme  | ✅      |
| TS-15 | `POST /simulations/calculate` sem token                           | Integração       | `401`                                 | Conforme  | ✅      |
| TS-16 | `POST /simulations/` salva a simulação                            | Integração       | `201` + `simulation_id`               | Conforme  | ✅      |
| TS-17 | `POST /simulations/` sem `company_id`                             | Integração       | `400`                                 | Conforme  | ✅      |
| TS-18 | `GET /simulations` lista da empresa                               | Integração       | `200` + 2 simulações                  | Conforme  | ✅      |
| TS-19 | `GET /simulations` sem `company_id`                               | Integração       | `400`                                 | Conforme  | ✅      |
| TS-20 | `DELETE /simulations/<id>` existente                              | Integração       | `200`, some da lista                  | Conforme  | ✅      |
| TS-21 | `DELETE /simulations/<id>` inexistente                            | Integração       | `404`                                 | Conforme  | ✅      |
| TS-22 | Cálculo em tempo real (parcela, total, juros)                     | Unitário (front) | resultado exibido sem salvar          | Conforme  | ✅      |
| TS-23 | Modalidade SAC destaca a "1ª Parcela"                             | Unitário (front) | rótulo "1ª Parcela" aparece           | Conforme  | ✅      |
| TS-24 | Formulário incompleto não exibe resultado                         | Unitário (front) | sem "Total a Pagar"                   | Conforme  | ✅      |
| TS-25 | Salvar chama o serviço de persistência                            | Unitário (front) | serviço chamado 1×                    | Conforme  | ✅      |
| TS-26 | Lista simulações salvas com seus parâmetros                       | Unitário (front) | linha com modalidade/valor            | Conforme  | ✅      |
| TS-27 | Excluir pede confirmação; "Cancelar" não chama o serviço          | Unitário (front) | serviço não chamado; modal fecha      | Conforme  | ✅      |
| TS-28 | Excluir confirmado chama o serviço com o id                       | Unitário (front) | serviço chamado com o id              | Conforme  | ✅      |
