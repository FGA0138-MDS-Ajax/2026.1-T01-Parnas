...
> **Status (auditoria):** em sua maior parte **NÃO está na develop**. O frontend de Contas/Transações ainda é mock (`Usecontas.jsx` usa `CONTAS_MOCK`), então a diferenciação visual, o badge "Gerado de conta" e o "Contas só pendentes" não existem; e o `GET /contas` retorna todas (não filtra "pendente" por padrão). O que existe é só backend: a FK `bill_id`/`id_conta` na transação e o `pay_bill` gerando a transação vinculada (veio pelo refactor de contas/transações, não por esta fix). Itens não entregues voltaram a ficar em aberto.
