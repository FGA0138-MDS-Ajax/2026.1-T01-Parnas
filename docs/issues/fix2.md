...
- [ ] Confirmar que a FK `id_conta` (nullable) existe na tabela `transacao`
- [ ] Criar migration caso a FK ainda não exista
...
## Tarefas Backend
- [ ] Revisar `PATCH /contas/<id>/quitar` para garantir que sempre cria a transação vinculada (`id_conta` preenchido)
- [ ] Garantir que `GET /contas` retorna apenas status "pendente" por padrão (parâmetro opcional para listar quitadas)
...
- [ ] PR aberto de `fix/diferenciacao-contas-transacoes` para `develop`
